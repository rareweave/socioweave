...
module.exports.executeBundlrQuery = async function* (tags) {
    for await (let bundlrGateway of config.gateways.bundlr) {
        let hasNextPage = true;
...
module.exports.fetchTxContent = async function (txId) {
    ...
    if (fromCache) { return fromCache }
    let fromGateway = await fetch(config.gateways.arweaveGateway + txId).catch(e => null).then(res => res ? res.arrayBuffer().catch(() => {
